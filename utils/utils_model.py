...
def test_mode(model, L, N, mode=0, refield=32, min_size=256, sf=1, modulo=1):
    '''
    # ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    # Some testing modes
    # ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    if mode == 0:
        ...
        E = test_pad(model, L, N, modulo)
    elif mode == 2:
        E = test_split(model, L, N, refield, min_size, sf, modulo)
    elif mode == 3:
        E = test_x8(model, L, N, modulo) # modulo=1
    elif mode == 4:
        E = test_split_x8(model, L, N, refield, min_size, sf, modulo)
    elif mode == 5:
        E = test_onesplit(model, L, N, refield, min_size, sf, modulo)
    return E
...
# pad (1)
def test_pad(model, L, N, modulo=16):
    h, w = L.size()[-2:]
...
# split (function)
def test_split_fn(model, L, N, refield=32, min_size=256, sf=1, modulo=1):
    '''
    model:
    ...
    if h * w <= min_size**2:
        ...
        Ns = [N[..., top, left], N[..., top, right], N[..., bottom, left], N[..., bottom, right]]
        # print("h * w > min_size**2, slice")
        if h * w <= 4*(min_size**2):
            Es = [model(Ls[i], Ns[i]) for i in range(4)]
        else:
            Es = [test_split_fn(model, Ls[i], Ns[i], refield=refield, min_size=min_size, sf=sf, modulo=modulo) for i in range(4)]
...
# split (2)
def test_split(model, L, N, refield=32, min_size=256, sf=1, modulo=1):
    E = test_split_fn(model, L, N, refield=refield, min_size=min_size, sf=sf, modulo=modulo)
    return E


# x8 (3)
def test_x8(model, L, N, modulo=1):
    E_list = [test_pad(model, util.augment_img_tensor(L, mode=i), util.augment_img_tensor(N, mode=i),
...
# split and x8 (4)
def test_split_x8(model, L, N, refield=32, min_size=256, sf=1, modulo=1):
    E_list = [test_split_fn(model, util.augment_img_tensor(L, mode=i), util.augment_img_tensor(N, mode=i), refield=refield, min_size=min_size, sf=sf, modulo=modulo)
              for i in range(8)]
...
# split only once (5)
def test_onesplit(model, L, N, refield=32, min_size=256, sf=1, modulo=1):
    '''
    model:
    L: input Low-quality image
    refield: effective receptive filed of the network, 32 is enough
